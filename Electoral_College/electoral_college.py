#Names(Me, Partner1, Partner2): Shima Abdulla, Christian Escobar, Sean Delaney

from argparse import ArgumentParser
import sys


def get_winner(electer_dict, to_dict):
    """Determines the winning party of the elector college
    Arguements: Requires a dictionary of the number of electors per state and a dictionary of the residing party.
    Returns: It returns a tuple of the winning candidate and how many votes they recieved
    """
    Republican_vote = 0
    Democrate_vote = 0
    for state in to_dict:
        if to_dict[state] == "R":
            Republican_vote += electer_dict[state]
        else:
            Democrate_vote += electer_dict[state]
    if Republican_vote > Democrate_vote:
        winner = ("R" ,Republican_vote)
    else:
        winner = ("D" ,Democrate_vote )
